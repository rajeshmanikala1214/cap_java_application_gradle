...
Run on Gradle 8.14.3 with a real JDK 21 (SapMachine 21.0.7).

| What | Result |
|---|---|
| Gradle wrapper | generated by Gradle 8.14.3 itself; `distributionSha256Sum` pinned so the wrapper validates its own download |
| Both modules configure without error | `gradle tasks --all` and `clean build --dry-run` succeed |
| Full `clean build` task graph and ordering | resolves to clean/cdsClean → installNode → npmCi → cdsResolve → cdsBuild → cdsGenerate → compileJava → processResources → jar/bootJar → test → jacocoTestReport → npmCiSidecar → startMtxSidecar → integrationTest → stopMtxSidecar |
| `npmCi` | executed; installed `@sap/cds-dk` 9.9.2 from `package-lock.json` |
| `cdsResolve` extraction | executed against a jar shaped like a CAP Java feature jar. Extracted models from both `cds/…` and `META-INF/cds/…`, wrote the `package.json` package marker, and correctly skipped `com/sap/cds/*.class` and single-segment entries |
| `cdsResolve` → `cdsBuild` chain | `cds build` compiled the model using **only** the jar-extracted models, with no hand-placed files: jar → `node_modules/com.sap.cds/<artifact>/` → CDS compiler → `csn.json` |
| `cdsBuild`, all three commands | executed through Gradle; produced `edmx/csn.json`, `schema-h2.sql` and `swagger/openapi.json` at the paths the POM expected, with no `gen/` leak |
| `cdsBuild` incremental | second invocation reports `UP-TO-DATE` |
| `cdsGenerate` entry-point discovery | executed against a stub generator jar. The main class was read from the jar manifest rather than hardcoded, and the invocation ran through `javaexec` |
| `cdsGenerate` argument substitution | `{csn}`, `{out}` and `{basePackage}` resolved to the correct absolute paths and base package |
| `cdsGenerate` output handling | generated file detected and counted; the "produced no .java files" guard is in place |
| `cdsGenerate` incremental | second invocation reports `UP-TO-DATE` |
...
clean — has been executed. What has not been executed is the Java half, and the
real generator.
