...
![CI status](https://github.com/SAP-samples/cloud-cap-samples-java/workflows/Java%20CI%20with%20Gradle/badge.svg)
...
CDS build steps (see [MIGRATION.md](MIGRATION.md) for the Maven → Gradle mapping):

- Install Node.js in a pinned version (`gradle/node.gradle`).
- Install the [@sap/cds-dk](package.json) version pinned in `package.json`.
- Execute the project's CDS commands (`gradle/cds.gradle`).
...
Make sure you have set up a development environment (that means, you’ve installed the CDS Compiler, Java 21, and Node.js 22 — Gradle ships with this repository as a wrapper, so no Gradle installation is needed) [as described here](https://cap.cloud.sap/docs/java/getting-started).
...
> [!NOTE]
> Please note that some IDEs may interfere with their autobuild when launching the application from the CLI using Gradle. Therefore, please ensure that no IDEs are running in parallel or launch the application natively from your preferred IDE as described below.
...
```
cds bind --exec -- ./gradlew clean build -Dspring.profiles.active=cloud
```
...
1.  Import the project using **File > Import > Existing Gradle Project**.
...
1.  To **compile** the project, run `./gradlew clean build` in the repository root, or use your IDE's Gradle view.

    This also compiles the CDS artifacts and generates the `cds.gen.*` sources.
...
```bash
./gradlew clean :srv:bootJar
```

```bash
pack build ${YOUR_CONTAINER_REGISTRY:?}/bookshop-srv \
        --path srv/build/libs/*-exec.jar \
